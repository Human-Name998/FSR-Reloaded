...
public class FSR2Pipeline {
    ...
    private static final String DEPTH_CLIP_CS = """
        ...
        float computeDepthClip(ivec2 pos) {
            ...
            // Use view-space Z ratio instead of device-depth epsilon for the same
            // reason as ACCUMULATE_CS: device-depth fluctuates under jitter at far plane.
    ...
    private static final String ACCUMULATE_CS = """
        ...
        void main() {
            ...
            // ---- DEBUG VISUALIZATION (replaces output when uDebugMode > 0) ----
            if (uDebugMode > 0) {
                vec3 dbg = vec3(0.0);
                if (uDebugMode == 1) {
                    // Sky/cloud: green=sky, blue=cloud, black=terrain
                    dbg = isCloud ? vec3(0.0, 0.0, 1.0) : (isSky ? vec3(0.0, 1.0, 0.0) : vec3(0.0));
                } else if (uDebugMode == 2) {
                    // isNewSample: white=new, black=accumulated
                    dbg = isNewSample ? vec3(1.0) : vec3(0.0);
                } else if (uDebugMode == 3) {
                    // Motion vectors: hue=direction, brightness=magnitude
                    ...
                    // Accumulation weight from lock status (0.0-1.0)
